...
pub fn compile_with_config(config: &Args, source: &str) -> Result<()> {
    ...
    if let Some(lexer) = lexer {
        if let OutputFormatEnum::TOKENS = config.output_format {
            ...
            std::fs::write(output_path, tokens.iter().flat_map(|s| s.chars()).collect::<String>()).unwrap();
        } else {
            //语法分析
            ...
            if let OutputFormatEnum::AST = config.output_format {
                ...
                return {
                    ast.print_to_with_root_name(output_file, config.input_path.to_str().unwrap().to_string()).unwrap();
                    Ok(())
                };
            }
            //类型检查和LLVM IR生成
